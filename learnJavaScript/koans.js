...
describe("the JavaScript language", function(){
  ...
  describe("has different types and operators that", function(){
    ...
    it("is not the same to compare by value than by reference ", function(){
        var a = [1,2,3];
        var b = [1,2,3];
        var one = {prop: 1};//homebrew
        var two = {prop: 1};//homebrew


        expect(one).toEqual(two);      //homebrew
        expect(one).not.toBe(two);    //homebrew
  ...
  describe("considers functions as first class citizens", function(){
    ...
    it("may return arrays that contains closures and so on", function(){
        function example(){
           return [function(args){ return [0, 9 + args] }]
        }
    ...
    it("can invoke functions indirectly using the special 'call'", function(){
        ...
        expect(f.call(this,1,1)).toEqual(2);
  ...
  describe("has multiple ways to define and create objects", function(){
    ...
    it("may create objects also with the module pattern", function(){
        function createObject(initialScore, color){
            ...
            this._getpoints = function(){ return this._points}

            this._incrementScoreIn = function(incrementPoints){
                this._points += incrementPoints;
            };

            return{
                color: _color,
                points: _getpoints,
                incrementScoreIn: _incrementScoreIn
            }
        }
